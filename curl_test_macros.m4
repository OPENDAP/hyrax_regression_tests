...
AT_ARG_OPTION_ARG([netrc],
    ...
    [echo "netrc file using default: ~/.netrc"; CURL_NETRC_FILE=""])
...
AT_ARG_OPTION_ARG([token],
    ...
    [echo "edl_auth_token set to: '-H \"$at_arg_token\"'"; EDL_AUTH_TOKEN="-H \"$at_arg_token\""])
...
m4_define([AT_CURL_RESPONSE_TEST], [dnl
    ...
    echo "##################################################################"
    echo "# AT_CURL_RESPONSE_TEST"
    echo "#"
    echo "edl_auth_token: '${EDL_AUTH_TOKEN}'"

    curl_command="curl -c ${abs_builddir}/cookies_file -b ${abs_builddir}/cookies_file ${EDL_AUTH_TOKEN} -L -K -"
    echo "curl_command: ${curl_command}"

    AS_IF([test -n "$baselines" -a x$baselines = xyes],
        [
        AT_CHECK([sed -e "s+@HYRAX_ENDPOINT_URL@+$HYRAX_ENDPOINT_URL+g" $input | ${curl_command} ],
            [0], [stdout])
...
m4_define([AT_CURL_DAP2_DATA_RESPONSE_TEST],  [dnl
    ...
    AS_IF([test -n "$baselines" -a x$baselines = xyes],
        ...
        AT_CHECK([
            ...
            curl -c $abs_builddir/cookies_file -b $abs_builddir/cookies_file -L -K - | 
            getdap -Ms -], [0], [stdout])
        PATCH_SERVER_NAME([stdout])
        AT_CHECK([mv stdout $baseline.tmp])
        ],
...
m4_define([AT_CURL_RESPONSE_PATTERN_MATCH_TEST], [dnl
    ...
    AS_IF([test -n "$baselines" -a x$baselines = xyes],
        [
        AT_CHECK([sed -e "s+@HYRAX_ENDPOINT_URL@+$HYRAX_ENDPOINT_URL+g" $input | curl -c $abs_builddir/cookies_file -b $abs_builddir/cookies_file -L -K -], [0], [stdout])
        PATCH_SERVER_NAME([stdout])
        AT_CHECK([mv stdout $baseline.tmp])
        ],
        [
...
m4_define([AT_CURL_RESPONSE_AND_HTTP_HEADER_TEST], [dnl
    ...
    AS_IF([test -n "$baselines" -a x$baselines = xyes],
        ...
        AT_CHECK([
            sed -e "s+@HYRAX_ENDPOINT_URL@+$HYRAX_ENDPOINT_URL+g" $input |
            curl -c $abs_builddir/cookies_file -b $abs_builddir/cookies_file -L -D $http_header -K -], [0], [stdout])
...
m4_define([AT_CURL_HTTP_HEADER_TEST], [dnl
    ...
    AS_IF([test -n "$baselines" -a x$baselines = xyes],
        ...
        AT_CHECK([
            sed -e "s+@HYRAX_ENDPOINT_URL@+$HYRAX_ENDPOINT_URL+g" $input |
            curl -c $abs_builddir/cookies_file -b $abs_builddir/cookies_file -L -D $http_header -K - > /dev/null], [0], [ignore])
